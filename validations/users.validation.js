...
const createSchema = Joi.object({
    password: Joi.string().pattern(passwordRegex).required().messages({
        ...
        'string.pattern.base': 'Password must have at least one digit, one uppercase, one lowercase, one special character, and a minimum of 8 total characters.',
        'any.required': 'Password is required.'
    }),
    email: Joi.string().pattern(lauEmailRegex).required().messages({
        'string.base': 'Email must be a string.',
        'string.pattern.base': 'Email must be in the format firstname.lastname@lau.edu or @lau.edu.lb.',
